...
subroutine postik( &
    ...
    ! parameter(MAXDAT = 10000, MAXCUT = 100)


	! external variables (in)
	real, intent(in) :: UNEST, outpar, varred, zmin,zmax,ltpar,mtpar,utpar
    ...
    real, intent(in), dimension(nc,na) :: prob
    ...
    if(varred < 0.0 .OR. varred > 1.0) then
    ...
    if(iout == 2 .AND. outpar < zmin) then
		write(*,*) 'Invalid z-value for iout=2'
		error = 4
	end if
    if(iout == 2 .AND. outpar > zmax) then
		write(*,*) 'Invalid z-value for iout=2'
		error = 5
	end if
    ...
    if(ivol == 1 .AND. ivtyp == 1) varred = sqrt(max(varred,0.0))
	...
	do l=1, na
		...
		! Volume support correction with parameters (ivol,ivtyp,varred):
