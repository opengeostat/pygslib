...
real*8 function powint(xlow,xhigh,ylow,yhigh,xval,pow)
	...
	!-----------------------------------------------------------------------
    real*8 :: EPSLON, xlow,xhigh,ylow,yhigh,xval, pow
...
real*8 function backtr(vrgs,nt,vr,vrg,zmin,zmax,ltail,ltpar, utail,utpar)
    ...
    if(vrgs <= vrg(1)) then
        ...
        cdfbt  = gcum(vrgs)
        if(ltail == 1) then
            backtr = powint(dble(0.0),cdflo,zmin,vr(1),cdfbt,dble(1.0))
        else if(ltail == 2) then
            cpow   = 1.0 / ltpar
            backtr = powint(dble(0.0),cdflo,zmin,vr(1),cdfbt,cpow)
        endif
    ...
    ! Value in the upper tail?     1=linear, 2=power, 4=hyperbolic:
        ...
        cdfbt  = gcum(vrgs)
        if(utail == 1) then
            backtr = powint(cdfhi,dble(1.0),vr(nt),zmax,cdfbt,dble(1.0))
        else if(utail == 2) then
            cpow   = 1.0 / utpar
            backtr = powint(cdfhi,dble(1.0),vr(nt),zmax,cdfbt,cpow)
    ...
    ! Value within the transformation table:
        ...
        j = max(min((nt-1),j),1)
        backtr = powint(vrg(j),vrg(j+1),vr(j),vr(j+1),vrgs,dble(1.0))
    endif
...
subroutine bacnscore(vnsc, nd, transin, transout, nt, ltail,utail, ltpar,utpar, zmin,zmax, getrank, va, error)
	...
	!   powint   power law interpolation
