...
subroutine declus( &
				 ...
				 wtopt, vrop,wtmin,wtmax, xmin,ymin,zmin, xmax,ymax,zmax, error, &         ! out
	...
	! Inout
	real*8, intent(inout) :: cmax, xmin,ymin,zmin, &
    						 xmax,ymax,zmax
	integer, intent(inout) :: MAXCEL
    ...
    logical :: min
	real*8 :: wt(nd)
    integer :: index(nd), lp, i, kp, xcs, ycs, zcs,vrcr, & 
    ...
    do lp=1,ncell+1
        ...
        if(ncellt > real(MAXCEL) .and. MAXCEL>1 ) then
